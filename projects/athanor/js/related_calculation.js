...
let draw_a_coordinate = function(svg, current_canvas_contain, text_canvas, coordinate_data, main_canvas_object, resize){
  ...
  function vo_mouse_over(e, d){
    console.log("Over", showHintPosIn);
    showHintPosIn = true;
  ...
  let showXValueTooltip = false;
  let showYValueTooltip = false;
  this.activate_value_tooltip = function (value){
    if(value === 'x'){
      showXValueTooltip = true;
    }
    if(value === 'y'){
      showYValueTooltip = true;
    }
    if(value === 'all'){
      showXValueTooltip = true;
      showYValueTooltip = true;
  ...
  let move_optimization = function(status, current_move_list, change_xy, mouse_point, current_canvas_position, current_vo_id){
    ...
    if (status.fast){
      ...
      if (is_point_in_current_area(mouse_point, main_canvas_object)){
        console.log("In of current area")
        if (allowOverlap && status.main_direction !== coordinate_data.groups[0].order && current_vo_id in coordinate_data.groups[0].obj_group_dict){
          coordinate_data.groups[0].order = status.main_direction
          main_canvas_object.dragOderStatus = 1 - (main_canvas_object.dragOderStatus === undefined ? 0 : main_canvas_object.dragOderStatus);
          console.log("order changed", main_canvas_object.dragOderStatus)
        }
  ...
  this.simple_change_order_list = function(point, vid_list){
    ...
    console.log("useful_vid: ", useful_vid);
    console.log("useful_group_index: ", useful_group_index);
  ...
  this.move_to_bottom = function(target_number) {
    let order_list = coordinate_data.groups[0].order_list;
    let index = order_list.indexOf(target_number);
    if (index > -1) {  // 존재하는 경우만 수행
        order_list.splice(index, 1);  // 해당 요소 제거
        order_list.push(target_number);  // 마지막에 추가
    } else {
        console.warn("Target number not found in order_list:", target_number);
    }
    console.log("Updated order_list:", order_list);
    update_hard_larger_list();  // 변경 사항 반영
  }
