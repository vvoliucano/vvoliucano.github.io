let draw_axis = function (canvas, axis, current_canvas_object) {
    ...
    if (zoomEnabled) {
      ...
      .call(d3.zoom()
        .filter(function (event) {
          return event.type !== 'dblclick'; // 더블 클릭을 줌 이벤트에서 제외
        })
        .on('start', zoom_axis_start)
        .on("zoom", zoom_axis_ing)
        .on('end', function (e, d) {
  ...
  function sorted_axis(e, sort_by) {
    ...
    if (axis.scale_type === "quantize") {
      ...
      const sort_by_list = ['min_value', 'max_value', 'diff', 'color', 'opacity']
      
      let chosen_attr;
      if (sort_by_list.includes(sort_by)) {
        chosen_attr = sort_by;
      } else {
        chosen_attr = choose_max_difference(value_array)
      }
      
      console.log('chosen_attr', chosen_attr);

      if(chosen_attr === 'opacity') {
        value_array.sort((item1, item2) => - item1.visual_object[chosen_attr] + item2.visual_object[chosen_attr])
      } else {
  ...
  function get_sorted_value() {
    ...
    console.log('should_value', should_value);
    console.log('!!! axis.value_range !!!', axis.value_range);
    let value_array = axis.value_range.map(function (d) { let current_item = new Array(); current_item['value'] = d; console.log('current_item', current_item.diff); return current_item })
    console.log('value_array000', value_array)
    console.log("control_coordinate 순서:", current_axis_object.control_coordinate.map(d => d.control_point));
    ...
    current_axis_object.control_coordinate.forEach(function (d) {
      ...
      visual_object.forEach(function (vo) {
        ...
        console.log('value_array5678', value_array);
